set -x
gfortran -m64 -std=legacy -finit-real=zero -finit-integer=0 -fbounds-check -w -static -o zchex-lin-gfort64.exe zchex.for zpass1.for zpass2.for zutilty_sub.for zdaniel_new.for zvaxutl.for zerr_suite.for dan_iod77.for
gfortran -m64 -std=legacy -finit-real=zero -finit-integer=0 -fbounds-check -w -static -o danlo-lin-gfort64.exe danlo.for
gfortran -m64 -std=legacy -finit-real=zero -finit-integer=0 -fbounds-check -w -static -o zorder-lin-gfort64.exe zorder.for zvaxutl.for zutilty_order.for

#cp -p zchex-lin-gfort64.exe zchex-lin-2019-03-06/zchexl.exe; exit
ls -la *.exe
...
cp -p  ../release/* .
mv     ../*.exe .
cp -p danlo-lin-gfort64.exe  danlol.exe
cp -p zchex-lin-gfort64.exe  zchexl.exe
cp -p zorder-lin-gfort64.exe zorderl.exe
...
zip -r zchex-lin64-`date +%F`.zip zchex-lin-`date +%F`
